...
export const useMNISTStore = create<MNISTWorkflowData & {
  ...
  init: (id, taskId = "mnist_cnn") => {
    ...
    if (get().sessionId === id) { set({ experimentResult: null, uploadInference: null, trainingCurve: [], currentEpoch: 0, totalEpochs: 0, isTraining: false, resultFingerprint: "", aiAnalysis: null, trainingCompletedAt: 0 }); return; }
