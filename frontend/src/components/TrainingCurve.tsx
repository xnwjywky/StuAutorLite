      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={160}>
          <LineChart data={displayData.length > 0 ? displayData : [{ epoch: 1 }]} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            <Tooltip formatter={(v: number) => v?.toFixed(4) || '-'} contentStyle={{ backgroundColor: "var(--app-card-bg)", borderColor: "var(--app-divider)", color: "var(--app-text)", borderRadius: 8, fontSize: 12 }} />
            <Legend iconType="line" wrapperStyle={{ fontSize: 11 }} />
            {hasData && <Line type="monotone" dataKey="train_loss" name="训练损失" stroke="#3964fe" strokeWidth={2} dot={{ r: 2 }} connectNulls />}
            {hasData && <Line type="monotone" dataKey="val_loss" name="验证损失" stroke="#f7ad31" strokeWidth={2} dot={{ r: 2 }} strokeDasharray="5 5" connectNulls />}
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={160}>
          <LineChart data={displayData.length > 0 ? displayData : [{ epoch: 1 }]} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            <Tooltip formatter={(v: number) => `${(v * 100).toFixed(1)}%`} contentStyle={{ backgroundColor: "var(--app-card-bg)", borderColor: "var(--app-divider)", color: "var(--app-text)", borderRadius: 8, fontSize: 12 }} />
            <Legend iconType="line" wrapperStyle={{ fontSize: 11 }} />
            {hasData && <Line type="monotone" dataKey="train_acc" name="训练准确率" stroke="#22c55e" strokeWidth={2} dot={{ r: 2 }} connectNulls />}
            {hasData && <Line type="monotone" dataKey="val_acc" name="验证准确率" stroke="#f25a5a" strokeWidth={2} dot={{ r: 2 }} strokeDasharray="5 5" connectNulls />}
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
