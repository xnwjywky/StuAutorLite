...
function Stage5() {
  ...
  const sampleGrids = displayRuns.length > 0 ? (displayRuns[0] as any).test_grids?.slice(0, 8) || [] : [];
  ...
  return (
    <StageContainer step={3} title="运行实验" actions={<div className="flex gap-3 w-full justify-between"><button className="btn-secondary" onClick={() => store.setStage("EXPERIMENT_DESIGNED")}>← 上一步</button><button className="btn-primary" onClick={() => store.setStage("RESULT_ANALYZED")} disabled={!result}>查看结果 → 分析</button></div>}>
      ...
      {/* 像素图形预览 */}
      {sampleGrids.length > 0 && (
        <div className="card">
          <h3 className="font-semibold text-gray-700 mb-3 text-sm">第 {store.selectedTrial} 组测试样本</h3>
          <div className="flex flex-wrap gap-2 justify-center">
            {sampleGrids.map((g: number[][], i: number) => (
              <ShapeGrid key={i} grid={g} label={SHAPE_NAMES[(displayRuns[0] as any).test_labels?.[i]] || ""}
                predicted={SHAPE_NAMES[(displayRuns[0] as any).predictions?.[i]] || ""}
                correct={(displayRuns[0] as any).test_labels?.[i] === (displayRuns[0] as any).predictions?.[i]} animate={true} />
            ))}
          </div>
        </div>
      )}
...
function generateMock(store: ReturnType<typeof useShapeRecogStore.getState>) {
  const algos = store.selectedAlgorithms.length > 0 ? store.selectedAlgorithms : ["TEMPLATE", "PIXEL_KNN", "FEATURE", "RANDOM"];
  const runs: any[] = []; const accuracies: Record<string, number> = { TEMPLATE: 0.95, PIXEL_KNN: 0.90, FEATURE: 0.85, RANDOM: 0.33 };
